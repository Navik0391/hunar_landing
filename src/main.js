...
// Логика снегопада из иконок
function initSnowfall() {
  const container = document.getElementById('snowfall-container');
  // Превентивная защита от дублирования при горячей перезагрузке (HMR)
  if (!container || container.children.length > 0) return;

  const icons = [
    '/icons/briefcase.svg',
    '/icons/broom.svg',
    '/icons/business-product-supplier.svg',
    '/icons/camera-retro.svg',
    '/icons/car.svg',
    '/icons/graduation-cap.svg',
    '/icons/hammer.svg',
    '/icons/help-headphones-customer-support-human.svg',
    '/icons/laptop-code.svg',
    '/icons/mobilephone-action-settings.svg',
    '/icons/people-carry.svg',
    '/icons/programming-user-code.svg',
    '/icons/settings-hammer.svg',
    '/icons/shipping-fast.svg',
    '/icons/taking-pictures-man.svg',
    '/icons/tools.svg'
  ];

  const maxIcons = 30; // Количество одновременно падающих иконок на экране

  for (let i = 0; i < maxIcons; i++) {
    createSnowflake(container, icons);
  }
}

function createSnowflake(container, icons) {
  if (!document.getElementById('snowfall-container')) return; // защита если секция удалена
  ...
  // Рандомизируем параметры анимации
  const randomLeft = Math.random() * 100; // Позиция по горизонтали (0% - 100%)
  const randomSize = Math.random() * 30 + 20; // Размер от 20 до 50px
  const randomDuration = Math.random() * 15 + 15; // Длительность падения от 15 до 30 сек (плавнее)
  const randomDelay = Math.random() * 15; // Задержка начала от 0 до 15 сек
  
  img.style.left = `${randomLeft}%`;
  img.style.width = `${randomSize}px`;
  img.style.height = `${randomSize}px`;
  
  // animationDuration: [время fall], [время sway]
  img.style.animationDuration = `${randomDuration}s, ${randomDuration / 2}s`;
  img.style.animationDelay = `${randomDelay}s, ${randomDelay}s`;
  
  container.appendChild(img);
  
  // Когда иконка долетает до конца, пересоздаем ее для бесконечного эффекта
  setTimeout(() => {
    img.remove();
    createSnowflake(container, icons);
